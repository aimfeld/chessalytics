...
- Issue: several backend services and frontend pages are far past the documented soft/hard limits (soft 100 / hard 200 *logic* LOC per function, but the files below are multi-thousand-line modules holding many such functions). `app/services/endgame_service.py` alone has functions like `_iterate_clock_rows` (~163 lines) and `_get_endgame_performance_from_rows` (~215 lines) by raw line count, and the whole file is 3,797 lines.
- Files: `app/services/endgame_service.py` (3,797 lines), `app/repositories/train_repository.py` (3,006 lines), `app/services/eval_apply.py` (2,792 lines), `app/services/tactic_detector.py` (2,587 lines), `app/services/insights_llm.py` (2,578 lines), `app/repositories/library_repository.py` (2,519 lines); frontend `frontend/src/pages/Analysis.tsx` (4,370 lines), `frontend/src/hooks/useBotGame.ts` (1,662 lines), `frontend/src/lib/engine/workerPool.ts` (1,451 lines), `frontend/src/pages/Openings.tsx` (1,376 lines)
...
- Fix approach: per CLAUDE.md guidance — split along pipeline-stage or data-shaping-hook seams when touched, rather than an unscoped big-bang refactor; `_get_endgame_performance_from_rows` and `_iterate_clock_rows` in `endgame_service.py` are the most immediate hard-limit-breaching candidates if that file is next touched.
