...
- Issue: several frontend pages/hooks/modules are far past the documented soft/hard limits (soft 100 / hard 200 *logic* LOC per function) — these are multi-thousand-line modules holding many such functions, and unlike the backend (see History below) there is no automated gate enforcing the limit on the frontend yet.
- Files: `frontend/src/pages/Analysis.tsx` (4,370 lines), `frontend/src/hooks/useBotGame.ts` (1,662 lines), `frontend/src/lib/engine/workerPool.ts` (1,451 lines), `frontend/src/pages/Openings.tsx` (1,376 lines)
...
- Fix approach: per CLAUDE.md guidance — split along data-shaping-hook or desktop/mobile-renderer seams when touched, rather than an unscoped big-bang refactor; a follow-up phase should also enable eslint `complexity` / `max-depth` gates for the frontend, mirroring what Phase 214 did for the backend with ruff `C901`/`PLR0912`/`PLR0915`, `complexipy` and `scripts/check_function_size.py`.
- History: the six backend god files this entry previously listed were fully decomposed in Phase 214 (Backend God-File Decomposition, completed 2026-09-03) with zero behavior change; this entry now tracks only the remaining frontend debt.
