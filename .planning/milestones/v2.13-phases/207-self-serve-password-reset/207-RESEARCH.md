...
- ~~**D-04 — Google-only accounts (empty `hashed_password`) get the standard reset link.**~~ **REVERSED 2026-08-08 — see the correction block below.**

> ### ⚠ D-04 REVERSED 2026-08-08 (operator decision, post-planning)
>
> **Password reset is only for accounts that HAVE a password. An account with an empty `hashed_password` receives NO reset email.** This inverts the decision recorded throughout this document. Where this file and `207-01-PLAN.md` disagree, **the PLAN is authoritative.**
>
> **The eligibility predicate is `hashed_password != ''` (credential state) — NOT "has no linked Google account".** Prod, measured 2026-08-08 (520 accounts, fully accounted for):
>
...
> | Guests | `''` | no | 304 | no |
>
> Gating on an `oauth_account` row would strand **125 of the 172** eligible accounts (73% of the target population), all holding real `$argon2id$` hashes. That regression is what Test 15 exists to catch.
>
> **Sections of this document superseded by the reversal — do not act on them:**
> - The Summary (§ line 9) clause "so D-04 needs zero special-casing".
> - **Pitfall 4 in full** (§ Common Pitfalls). It instructs you to treat `if not user.hashed_password` as scope creep and "flag it for removal". **That guidance is now exactly backwards** — the check is the phase's mandatory eligibility gate. Pitfall 4's *technical* finding remains true and useful (Argon2 hashes `""` fine, so the library would not break); it is the *conclusion* that is void.
> - The RESET-05 rows in § Phase Requirements and § Validation Architecture ("completes the flow and can then log in by either method").
>
> The empty-hash comparison is now required at **exactly one site** (the `on_after_forgot_password` gate) and must be read as *"has no password set"* (credential state), never as *"is a Google/SSO account"* (account type). Deriving eligibility from `oauth_account`, `oauth_accounts`, `is_guest`, or a hash-format/prefix check is prohibited.
>
> Guests share the empty hash and are excluded by the same predicate, so no separate `is_guest` check is needed — incidental, not a reason for the design. D-07's assessment stands: the guest path was already unreachable.
...
| RESET-05 | **REVISED — D-04 reversed.** Eligibility is credential state: an account with a password AND a linked Google account (125 in prod) still resets normally; an empty-`hashed_password` account gets no email while the response stays byte-identical. Proven by a test that goes red if eligibility is derived from `oauth_account`. | The library-capability finding (Argon2 hashes `""`, `_update` rewrites the hash) is still accurate but is no longer the requirement — see the D-04 correction block near the top |
...
### Pitfall 4: ~~Building a special-case branch for Google-only (empty-hash) accounts~~ — **CONCLUSION VOID (D-04 reversed 2026-08-08)**

> **Do NOT follow this pitfall's advice.** It tells you to flag `if user.hashed_password == ""` for removal as scope creep. That check is now the phase's **mandatory eligibility gate** (`207-01-PLAN.md` Task 1). The technical finding below is still correct and worth reading — Argon2 hashes `""` fine, so the library would not break on an empty-hash account — but the library *being able* to reset such an account is no longer a reason to let it. Eligibility is now a product decision, not a library capability. Retained for its verified library detail only.
...
| RESET-05 | **REVISED — D-04 reversed.** Dual account (password + `oauth_account` row) still resets; empty-hash account gets zero sends with an indistinguishable response | integration | Construct both fixtures (dual: real hash + an `oauth_account` row; ineligible: `hashed_password=""` mirroring `guest_service.py:156`). Assert the dual account completes forgot→reset→login, the empty-hash one triggers zero sends, and responses are captured-and-compared rather than hard-coded. Mutation: gate on any `oauth_account` row → the dual test goes red | ❌ new |
