...
| 4 | BENCHLANE-04 (SC4) — second backend on :8001 against benchmark DB produces full pipeline output end to end on a tranche | ✗ FAILED | Live DB: `game_positions` has 0 non-NULL `best_move`, 0 non-NULL `pv` (of 50M+ rows); `game_best_moves` has 0 rows. Alembic head matches (`0ac0176294fd` on both 5433 and repo). The end-to-end proof was deferred from 212-01 to end-of-phase UAT and never executed — see gap. |
...
| `docs/benchmark-lane-runbook.md` | Operator launch runbook | ⚠️ ORPHANED CLAIM | Exists and is otherwise accurate, but line 112's "scopes every lottery lane" claim is false — see truth 7 |
...
| `reports/benchmark-lane/benchmark-lane-classical-*.md` | SC6's row-count record artifact | ✗ MISSING | Directory does not exist |
