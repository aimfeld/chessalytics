...
- **Keep functions small and shallow.** The strongest signals are nesting depth and branching density; raw LOC is a cheap proxy.
  ...
  - Backend measurement tools: `uv run python scripts/check_function_size.py app/ --fail-over-depth 4 --fail-over-loc 200` (nesting depth + logic LOC), `uv run ruff check .` (`C901`/`PLR0912`/`PLR0915`, baselined per-file-ignores), `uv run complexipy app/ --max-complexity-allowed 15 --failed` (cognitive complexity, report only). See `docs/dev-tooling.md` for details and pragma usage.
  - Frontend measurement tools: `npm run lint` (from `frontend/`) gates `complexity`/`max-depth`/`max-statements` at `error`, baselined per pre-existing breach; `npm run lint:cognitive` reports Sonar cognitive complexity, not gated. See `docs/dev-tooling.md` for detail.
