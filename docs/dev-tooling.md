...
- **Frontend complexity rules** — `complexity` (max 15), `max-depth` (max 4) and `max-statements` (max 100) are enabled at `error` inside `frontend/eslint.config.js`'s `**/*.{ts,tsx}` block, so `npm run lint` gates them in CI. Scoped to TypeScript only because `eslint .` also walks the vendored engine bundles under `frontend/public/` (Stockfish/Maia/ONNX runtime), which would fail these rules heavily. `max-statements` is the logic-LOC analog rather than `max-lines-per-function` — a JSX return tree is one statement, honouring CLAUDE.md's "excluding the returned JSX tree" carve-out. Every pre-existing breach is baselined in one labelled Phase 215 override region that only ever shrinks. Check a file's real state with `npx eslint --no-inline-config --rule 'complexity: ["error", 15]' <path>` — the CLI flag defeats the baseline region.
- **`npm run lint:cognitive`** — `sonarjs/cognitive-complexity` at threshold 15, report-only, not part of `npm run lint` and not in CI, with its own config file `frontend/eslint.config.sonarjs.mjs`.
