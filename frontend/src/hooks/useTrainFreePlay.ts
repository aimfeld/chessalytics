 ...
 * the parent's rank-1 PV. ONE exception (Phase 211, D-06): a non-best ROOT-ply
 * move that matches the SERVED vetted list (`FreePlaySeedEval.vettedMoves` —
 * the same `SolveResponse.vetted_moves` the reveal's "Also fine" row draws)
 * is badged with that entry's own server-computed quality, never re-derived
 * from evals, so the badge can never contradict the list (SEED-137 case 2).
 * Deliberately NO Maia (Phase 200 UAT): the Train loop
...
/**
 * Phase 211 (D-06, replacing Phase 205's mount-rank seam): the grading
 * engine's seed for `useTrainFreePlay`, threaded through `TrainSolveScreen`'s
 * `freePlaySeedEval`. Extends `FreePlayEval`'s three fields with the SERVED
 * vetted list (`SolveResponse.vetted_moves` — the same entries the reveal's
 * "Also fine" row and green arrows draw), so the FIRST freely played move —
 * when it lands on the puzzle's ROOT ply and matches a served entry — is
 * badged with that entry's own server-computed quality instead of a fresh,
 * independently-searched post-move eval that can disagree with the list
 * (SEED-137 case 2). See `currentQuality`'s root-only branch below for where
 * this is consumed. `vettedMoves` is REQUIRED here (unlike the OPTIONAL wire
 * field, which a pre-211 restored verdict genuinely lacks at runtime) — the
 * one nullish default that absorbs a missing/older-bundle value lives at
 * `TrainSolveScreen.tsx`'s hoisted `vettedMoves` memo, not here (D-10: a
 * second default would make that one untestable).
 */
...
/**
 * Phase 211 (D-06): frozen empty vetted-move array — referential stability so
...
export function useTrainFreePlay({
  ...
  // Phase 211 (D-06): the SERVED vetted list, or the shared frozen empty
  // array when no seed is present yet. Deliberately NOT
  // `seedEval?.vettedMoves ?? NO_VETTED_MOVES` here — `seedEval.vettedMoves`
  // is a REQUIRED field on `FreePlaySeedEval` (the one nullish default for a
  // missing value lives at the hoisted `vettedMoves` memo in
  // `TrainSolveScreen.tsx`, per D-10 — a second default here would make that
  // one untestable).
  ...
  const currentQuality = useMemo<TrainMoveQuality | null>(() => {
    ...
    // Phase 211 (D-06, replacing Phase 205's mount-rank lookup): when the
    // played move is the puzzle's ROOT ply (no parent move —
    // `currentNode.parentId === null`) and it matches a SERVED vetted move,
    // return that entry's own server-computed quality directly — the
    // reveal's "Also fine" row and this badge read the SAME key, so a move
    // the row calls fine can never be badged worse when played (SEED-137
    // case 2). The badge is deliberately NOT re-derived from evals here:
    // mixing this client engine's parent eval with the server key's deep
    // child eval would create a THIRD grader that agrees with neither side
    // — the trap the retired rank-line mechanism would have become at width
    // 1. Precedence: a terminal position (the rules) wins over the key, and
    // the engine's own top move still reads best, not good — both checked
    // above. Root-only on purpose: below the root, the parent and child
    // evals already come from ONE engine (this free-play engine itself),
    // which is already self-consistent — the key describes the PUZZLE
    // position, not any deeper one.
  ...
  }, [isExploring, currentNode, parentFen, fen, evalByFen, liveCp, liveMate, seedVettedMoves]);
